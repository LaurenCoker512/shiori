...
describeIfDb('GET /api/grammar-patterns/[id]/sentences — integration', () => {
  ...
  beforeAll(async () => {
    ...
    mockGetSession.mockResolvedValue(USER_1);
  });
  ...
  async function seedText(userId: number, title: string, parsedContent: unknown[]): Promise<number> {
    const result = await testPool.query<{ id: number }>(
      `INSERT INTO texts (user_id, title, raw_content, parsed_content)
       VALUES ($1, $2, 'raw', $3) RETURNING id`,
      [userId, title, JSON.stringify(parsedContent)],
    );
    return result.rows[0].id;
  }

  async function seedGrammarPattern(userId: number, pattern: string): Promise<number> {
    const result = await testPool.query<{ id: number }>(
      `INSERT INTO grammar_patterns (user_id, pattern, description_en, jlpt_level)
       VALUES ($1, $2, 'Description', 'N4') RETURNING id`,
      [userId, pattern],
    );
    return result.rows[0].id;
  }
  ...
  it('unauthenticated → 401', async () => {
    ...
    const response = await GET(makeGetRequest(), makeParams('1'));
    expect(response.status).toBe(401);
  });

  it('cross-user access → returns empty sentences (ownership filter applied)', async () => {
    const parsedContent = [{ sentence_index: 0, raw: '猫が食べた。', tokens: [] }];
    const textId = await seedText(1, 'User 1 Text', parsedContent);
    const patternId = await seedGrammarPattern(1, '〜た');
    await testPool.query(
      'INSERT INTO sentence_patterns (text_id, sentence_index, grammar_pattern_id) VALUES ($1, 0, $2)',
      [textId, patternId],
    );

    mockGetSession.mockResolvedValue(USER_2);
  ...
  it('returns sentences for a known grammar pattern ordered by title then sentence_index', async () => {
    ...
    const textId = await seedText(1, 'Test Title', parsedContent);
    const patternId = await seedGrammarPattern(1, '〜ていた');
  ...
  it('excludes NULL sentinel rows (grammar_pattern_id IS NULL)', async () => {
    ...
    const textId = await seedText(1, 'Test', parsedContent);
    const patternId = await seedGrammarPattern(1, '〜ていた');
